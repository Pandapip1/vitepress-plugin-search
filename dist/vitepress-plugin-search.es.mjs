...
const replaceMdSyntax = (e) => e.replace(/\[(.*?)\]\(.*?\)/g, "$1").replace(/(\*+)(\s*\b)([^\*]*)(\b\s*)(\*+)/gm, "$3"), match = (e, n) => {
  ...
  for (const l of r)
    ((s = n == null ? void 0 : n.allow) == null ? void 0 : s.length) > 0 && !match(`${e}/${l.name}`, n.allow) || ((o = n == null ? void 0 : n.ignore) == null ? void 0 : o.length) > 0 && match(`${e}/${l.name}`, n.ignore) || (l.isDirectory() && l.name != "node_modules" ? i = [...i, ...await getFileList(`${e}/${l.name}`, n)] : l.name.endsWith(".md") && i.push(`${e}/${l.name}`));
  return i;
...
`), path: n };
  ...
  return n.includes(".0") || (o += `#${slugify(s)}`), r ? {
    id: n,
    link: o,
    b: e.content,
    a: s,
...
(function(module) {
  (function _f(self) {
    ...
    t = Q.prototype, t.add = function(e, n, i) {
      ...
      for (let g = 0, k, R, j; g < f.length; g++) {
        let $;
        if (R = f[g], x(R) || ($ = R, R = $.field, e = $.query || e, n = $.limit || n), w)
          w[g] = this.index[R].searchAsync(e, n, $ || i);
        else {
          if (r ? k = r[g] : k = this.index[R].search(e, n, $ || i), j = k && k.length, p && j) {
            const _ = [];
            let B = 0;
            m && (_[0] = [k]);
            for (let z = 0, H, q; z < p.length; z++)
              H = p[z], (j = (q = this.l[H]) && q.length) && (B++, _[_.length] = m ? [q] : q);
            B && (k = m ? ma(_, n || 100, A || 0) : na(k, _), j = k.length);
          }
          if (j)
            o[y] = R, s[y++] = k;
          else if (m)
